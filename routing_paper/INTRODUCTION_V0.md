...
Our contribution is consequently a finite representation result rather than a new theory of mutation–selection balance or genotype–phenotype threshold dependence. A declared branch-product routing architecture induces an exactly countable hierarchy of genotype layers; a nested-chain bound reduces all lower-layer competition to the adjacent layer; and standard selection weighting then yields exact thresholds for two distinct forms of stationary occupancy. The accompanying representation and mutation-measure controls show precisely when those conclusions cease to be identified.
...
- Wilke et al. 2001 and later quasispecies/phenotypic-error-threshold work — neutral-network breadth and genotype–phenotype redundancy can shift evolutionary thresholds.
...
Final citation formatting and priority wording remain blocked on the dedicated source-level novelty pass.