...
where `theta` is the per-gain stationary tilt (`theta = a^(N-1)` in the declared Moran fitness-step parameterization), the full-gain routing genotype is a modal layer if and only if
...
4. a sharp condition for when the unique full-gain genotype becomes a stationary **mode** despite the combinatorial mass of lower-gain genotypes.
...
> A finite routing architecture with weakest-branch gain has an exact combinatorial entropy profile, and under standard origin-fixation selection the full adaptive phase becomes a stationary mode exactly when selection tilt crosses the sharp architecture-dependent threshold `2^k - 1`.
